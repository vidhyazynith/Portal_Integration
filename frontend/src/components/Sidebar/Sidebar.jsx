...
const CalcIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
    <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 2h2v2h-2V5zm3 14H8v-2h7v2zm3 0h-2v-2h2v2zm0-4H8v-2h10v2zm0-4H8V9h10v2zM8 7V5h2v2H8z"/>
  </svg>
);
...
const Sidebar = ({ activeSection, setActiveSection, onLogout }) => {
  ...
  const menuItems = [
    ...
    { id: 'reports', label: 'Reports & Billing', icon: <ChartIcon /> },
    {
      ...
      icon: <TaxIcon />,
      submenu: [
        { id: 'financial-years', label: 'Financial Years', icon: <CalendarIcon /> },
        { id: 'tax-regimes', label: 'Tax Regimes', icon: <RegimeIcon /> },
        { id: 'tax-slabs', label: 'Tax Slabs', icon: <SlabIcon /> },
        { id: 'deduction-limits', label: 'Deduction Limits', icon: <LimitIcon /> },
        { id: 'employee-tax-regime', label: 'Employee Regime', icon: <UserRegimeIcon /> },
        { id: 'employee-tax-declaration', label: 'Tax Declaration', icon: <DeclarationIcon /> },
        { id: 'tax-calculation', label: 'Tax Calculation', icon: <CalcIcon /> }
      ]
    },
  ];
  ...
  const handleIncomeTaxClick = () => {
    setIncomeTaxOpen(!incomeTaxOpen);
    if (!incomeTaxOpen && !menuItems.find(item => item.id === 'income-tax')?.submenu?.some(subItem => subItem.id === activeSection)) {
      setActiveSection('financial-years');
    }
  };
  ...
  const isIncomeTaxActive = menuItems.find(item => item.id === 'income-tax')?.submenu?.some(subItem => subItem.id === activeSection);
        ...
        <nav className="sidebar-nav">
          {menuItems.map(item => {
            ...
            if (item.id === 'income-tax') {
              return (
                <div key={item.id} className={`dropdown-container ${incomeTaxOpen ? 'open' : ''}`}>
                  <button
                    className={`nav-item dropdown-toggle ${isIncomeTaxActive ? 'active' : ''}`}
                    onClick={handleIncomeTaxClick}
                    data-tooltip={item.label}
                  >
                    <span className="nav-icon">{item.icon}</span>
                    <span className="nav-label">{item.label}</span>
                    <span className="dropdown-arrow">
                      {incomeTaxOpen ? <ChevronUpIcon /> : <ChevronDownIcon />}
                    </span>
                  </button>

                  <div className={`dropdown-menus ${incomeTaxOpen ? 'open' : ''}`}>
